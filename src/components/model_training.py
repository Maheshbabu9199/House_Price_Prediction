...
class ModelTraining:
    ...
    def run_pipeline(self, random_state=10):
        try:
            logging.warning("Model Training Started")
            
            self.split_data(random_state=10)  # custom function
    ...
    def split_data(self, random_state=42):
        try:
            ...
            self.train_data, self.test_data = train_test_split(data, test_size=self.config.test_size, random_state=random_state)
